...
let defineProperties = (target, values) => {
  let reducer = (map, key) => ({ ...map,
    [key]: { ...Object.getOwnPropertyDescriptor(values, key),
      "enumerable": false
    }
  });
...
defineProperties(Array.prototype, {
  ...
  reduceSkip(callback, initial) {
    ...
    for (let index = 0; index < this.length; index++) {
      ...
      if (!previousExists) {
        previousExists = true;
        if (arguments.length > 1) previous = Object.clone(initial);else {
          previous = value;
          continue;
        }
      }
...
class AdvancedError extends Error {
  ...
  static stringifyStack(error) {
    ...
    while (error instanceof AdvancedError && error.suppressed) errors.push(error = error.suppressed);

    let string = "";
    let lastStack;
    errors.forEach((error, index) => {
      let fullStack = error instanceof AdvancedError ? error.stackMap : errors[index - 1].suppressedStackMap;
      ...
      if (lastStack) {
        ...
        let lcsf = usedStack.softReverse().find((frame, index) => {
          ...
          return !compare || frame.methodName != compare.methodName || frame.file != compare.file;
        });
      ...
      usedStack.forEach(stack => {
        string += "\n\tat " + stack.methodName + " (" + stack.file + ":" + stack.lineNumber + ":" + stack.column + ")";
      });
      if (usedStack.length < fullStack.length) string += "\n\t... " + (fullStack.length - usedStack.length) + " more";
      lastStack = fullStack;
    });
    return string;
  }
  ...
  static parseStack(stackString) {
    ...
    return lines.reduce((stack, line) => {
      const parseResult = AdvancedError.parseChrome(line) || AdvancedError.parseWinjs(line) || AdvancedError.parseGecko(line) || AdvancedError.parseNode(line) || AdvancedError.parseJSC(line);
  ...
  constructor(message, suppressed) {
    if (message instanceof Error) [message, suppressed] = [undefined, message];
    super(); // if (!(this instanceof arguments.callee)) {
    ...
    _defineProperty(this, "displayedStack", void 0);

    _defineProperty(this, "description", void 0);

    _defineProperty(this, "stackMap", void 0);

    _defineProperty(this, "suppressed", void 0);

    _defineProperty(this, "suppressedStackMap", void 0);
...
_defineProperty(AdvancedError, "UNKNOWN_FUNCTION", '<unknown>');
...
let ArgumentsError = AdvancedError.extend("ArgumentsError"); //export {AdvancedError};