...
let generators = {
  "JS": {
    "blockToolsBuilder": ["", "\n\n", "", (map, shared) => {
      ...
      let endings = map.colour != 0 || map.style != "AUTO" || map.output !== undefined || map.preceding !== undefined || map.succeeding !== undefined;
      if (shared.blocks == undefined) shared.blocks = [map.name];else shared.blocks.push(map.name);
      ...
      let contentReducer = (prev, cur, skip) => {
        ...
        if (typeof cur == "string" || typeof cur == "undefined") {
          ...
          if (cur.instance == "input") prev.input = cur;else {
            prev.content = prev.content || [];
      ...
      for (let piece of content) {
        ...
        if (piece.input && piece.input.type != "dummy") {
          ...
          if (piece.input.check != null) code.append(`.setCheck("${piece.input.check.escape()}")`);
          if (piece.input.align != "LEFT") code.append(`.setAlign("${piece.input.align}")`);
        } else {
      ...
      if (map.output !== undefined) code.append(`.setOutput(${map.output ? `"${map.output.escape()}"` : ""})`);
      if (map.preceding !== undefined && map.succeeding === map.preceding) code.append(`.setBothStatements(${map.preceding ? `"${map.preceding.escape()}"` : ""})`);
      if (map.preceding !== undefined && map.succeeding !== map.preceding) code.append(`.setPreviousStatement(${map.preceding ? `"${map.preceding.escape()}"` : ""})`);
      if (map.succeeding !== undefined && map.succeeding !== map.preceding) code.append(`.setNextStatement(${map.succeeding ? `"${map.succeeding.escape()}"` : ""})`);
...
class BlocklyBlockBuilder {
  ...
  static loadBlocks() {
    ...
    let FieldDropdown = Builder.FieldDropdown; //Entities

    new BlockTools.Builder("builder_entity_block").appendValueInput("name").setCheck("string").setAlign("RIGHT").appendField("Block definition").appendValueInput("style").setCheck("style").setAlign("RIGHT").appendField("Input style").appendValueInput("hue").setCheck("hue").setAlign("RIGHT").appendField("Color").setColour(240).setNextStatement("field").register(); //Values

    new BlockTools.Builder("builder_value_text").appendDummyInput().appendField("\"").appendField(new BlockTools.Builder.FieldTextInput(""), "value").appendField("\"").setColour(300).setOutput("string").register();
    new Builder("builder_value_style").appendField(new FieldDropdown({
      "AUTO": "automatic",
      "INTERNAL": "internal",
      "EXTERNAL": "external"
    }), "value").setColour(300).setOutput("style").register();
    new Builder("builder_value_alignment").appendField(new FieldDropdown({
      "LEFT": "left",
      "CENTRE": "center",
      "RIGHT": "right"
    }), "value").setColour(270).setOutput("alignment").register();
    new Builder("builder_value_hue").appendField("hue").appendField(new FieldAngle(0, function (hue) {
      this.sourceBlock_.setColour(hue);
    }), "value").setColour(0).setOutput("hue").register(); //Text (Made In Editor!)

    new BlockTools.Builder("builder_text").appendValueInput("content").setCheck("string").appendField("Text").setColour(180).setBothStatements("field").register();
    new BlockTools.Builder("builder_input_dummy").appendDummyInput().appendField("New line").setColour(180).setBothStatements("field").register(); //Inputs

    new BlockTools.Builder("builder_input_value").appendValueInput("name").setCheck("string").setAlign("RIGHT").appendField("Input").appendValueInput("type").setCheck("type").setAlign("RIGHT").appendField("accepting").appendValueInput("align").setCheck("alignment").setAlign("RIGHT").appendField("aligned").setColour(180).setInputsInline(true).setBothStatements("field").register();
    new BlockTools.Builder("builder_input_statement").appendValueInput("name").setCheck("string").setAlign("RIGHT").appendField("Statement").appendValueInput("type").setCheck("type").setAlign("RIGHT").appendField("accepting").appendValueInput("align").setCheck("alignment").setAlign("RIGHT").appendField("aligned").setColour(180).setInputsInline(true).setBothStatements("field").register(); //Fields

    new BlockTools.Builder("builder_field_text").appendValueInput("name").appendField("Text field").setCheck("string").appendValueInput("default").appendField("defaulting to").setCheck("string").setColour(180).setBothStatements("field").setInputsInline(true).register();
    new BlockTools.Builder("builder_field_number").appendValueInput("name").appendField("Number field").setCheck("string").appendValueInput("default").appendField("defaulting to").setCheck("int").setColour(180).setBothStatements("field").setInputsInline(true).register(); //Type

    new Builder("builder_type").appendField(new FieldDropdown(...this.types.getConstructArgs()), "type").setColour(240).setOutput("type").register(); //Connections

    new Builder("builder_connection_output").appendValueInput("type").setCheck("type").appendField("Output").setColour(45).setPreviousStatement("field").register();
    new Builder("builder_connection_succeeding").appendValueInput("type").setCheck("type").appendField("Succeeding").setColour(45).setPreviousStatement("field").register();
    new Builder("builder_connection_preceding").appendValueInput("type").setCheck("type").appendField("Preceding").setColour(45).setPreviousStatement("field").register();
    new Builder("builder_connection_both").appendValueInput("preceding").setAlign("RIGHT").setCheck("type").appendField("Preceding").appendValueInput("succeeding").setAlign("RIGHT").setCheck("type").appendField("& Succeeding").setColour(45).setPreviousStatement("field").register();
  }
  ...
  static onActionsComplete() {
    ...
    this.blocklyEditor = Blockly.inject("editor-workspace", { ...common,
      "toolbox": this.toolbox.documentElement
    });
    this.blocklyPreview = Blockly.inject("preview-workspace", { ...common,
      "toolbox": null,
      //TODO: Make preview toolbox work!
      "trashcan": false,
  ...
  static updatePreviewWorkspace(code) {
    ...
    all = all.filter(name => !created.includes(name)); // console.debug("All:", all);
  ...
  static editorEventHandler(event) {
    if (!(event instanceof Blockly.Events.Ui)) {
      ...
      let recursiveFill = oldBlock => {
        ...
        if (oldBlock.getPreviousBlock()) conBlock.nextConnection.connect(newBlock.previousConnection);else oldBlock.outputConnection.targetConnection.connect(newBlock.outputConnection);
      };
  ...
  static inputEventHandler({
    ...
    if (condition) hov.classList.add("hide");else hov.classList.remove("hide");
  }

}

_defineProperty(BlocklyBlockBuilder, "blocklyEditor", void 0);

_defineProperty(BlocklyBlockBuilder, "blocklyPreview", void 0);

_defineProperty(BlocklyBlockBuilder, "codePreview", void 0);

_defineProperty(BlocklyBlockBuilder, "pageResolve", void 0);

_defineProperty(BlocklyBlockBuilder, "pinky", void 0);

_defineProperty(BlocklyBlockBuilder, "toolbox", void 0);

_defineProperty(BlocklyBlockBuilder, "getNext", block => ({
  "builder_text": "text",
...
_defineProperty(BlocklyBlockBuilder, "mapper", {
  "$main": {
    "$lit_content": [],
    "name": "name.value",
    "style": "style.value",
  ...
  "connection": {
    "$runme": function () {
      let add = {
        "builder_connection_output": {
          "output": "type.type"
        },
        "builder_connection_succeeding": {
          "succeeding": "type.type"
        },
        "builder_connection_preceding": {
          "preceding": "type.type"
        },
...
_defineProperty(BlocklyBlockBuilder, "types", void 0);

_defineProperty(BlocklyBlockBuilder, "generator", void 0);

_defineProperty(BlocklyBlockBuilder, "language", "JS");

_defineProperty(BlocklyBlockBuilder, "format", "blockToolsBuilder");
