...
namespace BlockTools {
  ...
  */
  ...
  export namespace Map {
    ...
    /**
     * Maps specific values of a block to an object using
     * paths (see getValues) or functions.
     */
  ...
  */
   ...
   * @returns XML Element.
   */
  export const createXml = (xml: string) =>
    Blockly.Xml.textToDom(`<xml>${xml}</xml>`);
   ...
   */
  export const obtainXml = (xml: string) =>
    Array.from(createXml(xml).childNodes).crumble();
   ...
   */
  export const map = (() => {
    let makeThis = (block: Blockly.Block, scope: Object, diagram: Object):
        Map.This => ({
      "block": block,
      "scope": scope,
      "diagram": diagram,
    ...
    let mapField = (key: string, mapper: Map.Mapper, block: Blockly.Block,
        map: Of<any>, mapScope: Of<any>, blockMapper: Map.BlockMapper) => {
    ...
    let mapBlock = (mapper: Map.Mapper, block: Blockly.Block, map?: Of<any>,
        mapScope?: Of<any>, blockMapper?: Map.BlockMapper) => {
      ...
      if (nextMap && nextBlock) {
        if (typeof nextMap == "function")
          nextMap =
            nextMap.call(makeThis(block, mapScope, map), nextBlock) as string;
  ...
  export namespace Map {
    ...
    export class MapperError extends AdvancedError {
      ...
      constructor(type: Error | "field" | "input", index?: number,
          path?: string) {
        if (type instanceof Error) {
          ...
          let reducer = (p: string | number, c) =>
            (typeof p == "string" ? p.length : p) + c.length + 1;
   ...
   *  type.
   */
  export const getValue = (block: Blockly.Block, path: string):
  ...
  */
  ...
  export class CodeGenerator {
    ...
    public constructor(mapper: Map.Mapper,
        blockType: string | ((blocks: Blockly.Block[]) => Blockly.Block[])) {
      this.mapper = mapper;
      this.blockType = blockType;
    }

    public generate(workspace: Blockly.WorkspaceSvg,
        generator: string | (CodeGenerator.Generator)):
        (Code | CodeGenerator.GeneratorError)[] {
      let blocks: Blockly.Block[] = workspace.getAllBlocks(true);
      ...
      let code = maps.map(([id, map]) => {
        ...
        timer.time(id);
        try {
          code = (generator as (valueMap: Of<any>, shared?: any) => Code)
            (map, shared);
        } catch (error) {
          code =
  ...
  export class Types {
    ...
    public updateDropdown =
        this.divert(function(this: Types, dropdown: Blockly.FieldDropdown):
          Entry<string>[] {
      let data =
        Object.entries(this.table).map((a) => a.reverse()) as Entry<string>[];
      let options = dropdown && dropdown.getValue() != "-1" ?
        [["Delete Type...", "-3"]] as Entry<string>[] : [];
    ...
    public validateDropdown =
      ...
      let switchObj = {
        ...
        "-3": () =>
          this.deleteType(dropdown.sourceBlock_.workspace, dropdown.getValue())
      }
     ...
     */
    public getConstructArgs():
        [() => Entry<string>[], (option: string) => void] {
    ...
    public buildToolbox(this: Types) {
      ...
      return list.flat() as Element[];
    }
    
    @bound
    public createType(this: Types, button?: Blockly.FlyoutButton):
        string | void {
      ...
      let position = Math.max(-1,
        ...Object.keys(this.table).map((int) => parseInt(int))) + 1;
    ...
    @bound
    public deleteType(this: Types,
        argument: Blockly.FlyoutButton | Blockly.Workspace, item?: string,
        confirmm?: boolean): boolean {
      if (argument instanceof Blockly.Workspace) {
        ...
        for (let block of argument.getAllBlocks(true)) {
          ...
          if (block.getField("type").getValue() == id)
            (block as Blockly.BlockSvg).dispose(false, true);
        }
        return true;
      }
  
      let recent = this.table
        [Math.max(...Object.keys(this.table).map((a) => parseInt(a)))];
      item = prompt("Enter the name of the type to delete...", recent);
      if ([null, ""].includes(item)) return false;
      item = Object.fromEntries(
        Object.entries(this.table).map((a) => a.reverse()))[item];
      ...
      let initFunc = function(this: Blockly.Block) {
        for (let builtInput of builder.inputs) {
          ...
          for (let field of builtInput.fields) {
            ...
            input.appendField(new Blockly[name](...field.getArguments()),
              field.name);
          }
        }
  
        this.setOutput(builder.output !== undefined, builder.output);
        this.setPreviousStatement(builder.previous !== undefined,
          builder.previous);
  ...
  export namespace Builder {
    ...
    export class Field<Contents, Validator = Contents> {
      ...
      protected validator: (this: Blockly.Field, value: Validator) =>
        Validator | void;
  
      public constructor(value: Contents,
          validator?: (this: Blockly.Field, value: Validator) =>
            Validator | void) {
        this.value = value;
      ...
      public constructor(options: ((this: FieldDropdown) => Of<string> |
        ...
        if (!(options instanceof Array) && typeof options != "function") {
          options = Object.entries(options)
            .map(([k, v]) => [v, k] as Pair<string, string>);
        ...
        super(options as ((this: FieldDropdown) =>
          Entry<string>[]) | Entry<string>, validator);
      }
